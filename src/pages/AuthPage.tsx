...
const AuthPage = () => {
  ...
  // If user is already authenticated, redirect away immediately
  useEffect(() => {
  ...
  }, [navigate, resumePendingAction]);

  // Listen for auth state changes (handles OAuth callback)
  useEffect(() => {
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, session) => {
      if (event === "SIGNED_IN" && session) {
        trackEvent(Events.LOGIN_SUCCESS, { provider: "session" });
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        ...
        if (error) throw error;
        trackEvent(Events.LOGIN_SUCCESS, { provider: "email" });
        // onAuthStateChange will handle redirect
      }
    } catch (err: any) {
      trackEvent(Events.LOGIN_ERROR, { error: err.message });
  ...
  const handleOAuth = async (provider: "google" | "apple") => {
    trackEvent(provider === "google" ? Events.CLICK_GOOGLE_LOGIN : Events.CLICK_APPLE_LOGIN);
    try {
      const { error } = await lovable.auth.signInWithOAuth(provider, {
        redirect_uri: window.location.origin,
      });
      if (error) {
        trackEvent(Events.LOGIN_ERROR, { provider, error: String(error) });
        toast.error("Não foi possível conectar. Tente novamente.");
      }
    } catch {
      trackEvent(Events.LOGIN_ERROR, { provider });
  ...
  if (checkingSession) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-[hsl(30,20%,12%)] via-[hsl(30,15%,10%)] to-[hsl(30,10%,8%)]">
  ...
  return (
    <div className="min-h-screen flex flex-col relative">
      ...
      <div className="relative z-10 flex-1 flex flex-col px-6 pt-10 pb-8">
        ...
        {/* Social buttons FIRST — reduce friction */}
        <div className="flex gap-3 w-full max-w-sm mx-auto mb-6">
          <button
            onClick={() => handleOAuth("google")}
            disabled={loading}
            className="flex-1 py-3 rounded-full bg-white/10 border border-white/15 text-white text-sm font-medium hover:bg-white/15 transition-colors flex items-center justify-center gap-2 active:scale-[0.97]"
          >
        ...
        {/* Divider */}
        <div className="flex items-center gap-3 w-full max-w-sm mx-auto mb-6">
          <div className="flex-1 h-px bg-white/10" />
          <span className="text-[11px] text-white/30">ou com e-mail</span>
          <div className="flex-1 h-px bg-white/10" />
        </div>
        ...
        <form onSubmit={handleEmailAuth} className="space-y-4 w-full max-w-sm mx-auto">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-full bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
