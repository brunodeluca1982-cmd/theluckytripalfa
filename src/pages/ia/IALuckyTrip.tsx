...
const IALuckyTrip = () => {
  ...
  useEffect(() => {
    ...
    const items = ctx.minha_viagem_items || [];
    const itemNames = items.map((i: any) => i.title).filter(Boolean);
    const neighborhoods = items.map((i: any) => i.neighborhood).filter(Boolean);
    const totalDays = ctx.travel_dates?.totalDays || 3;

    let prompt = `Monte um roteiro completo de ${totalDays} dias no Rio de Janeiro.`;

    if (itemNames.length > 0) {
      prompt += ` Minhas inspirações são: ${itemNames.join(", ")}.`;
      if (neighborhoods.length > 0) {
        prompt += ` Bairros de interesse: ${[...new Set(neighborhoods)].join(", ")}.`;
      }
      prompt += ` Use esses lugares como âncora e complete com hotel, restaurantes e experiências.`;
    } else {
      prompt += ` Escolha o melhor hotel, experiências e restaurantes automaticamente.`;
    }

    prompt += ` Organize por zona geográfica, com ritmo realista. Quero ver o plano pronto.`;
