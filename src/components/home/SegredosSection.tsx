    ...
    <>
      <section className="py-8 px-5">
        ...
        <p className="text-muted-foreground text-sm leading-relaxed mb-6">
          Descobertas escolhidas a dedo por Bruno De Luca.
        </p>
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide -mx-5 px-5">
          {secrets.map((item, i) => {
            ...
            if (isLocked) {
              return <LockedSecretCard key={item.id} item={item} onTap={() => setShowPaywall(true)} />;
            }
            return (
              <Link
                key={item.id}
...
const SecretCardContent = ({ item }: { item: LuckyListItem }) => (
  <>
    ...
    <div className="px-3 py-3">
      <p className="text-foreground text-sm font-medium leading-tight line-clamp-2">{item.nome}</p>
      <p className="text-muted-foreground text-[11px] mt-1 line-clamp-2">{item.meu_olhar}</p>
    </div>
  </>
);

const LockedSecretCard = ({ item, onTap }: { item: LuckyListItem; onTap: () => void }) => (
  <button
    onClick={onTap}
  ...
  >
    ...
    <div className="px-3 py-3">
      <p className="text-foreground text-sm font-medium leading-tight line-clamp-2 blur-[3px]">{item.nome}</p>
      <p className="text-muted-foreground text-[11px] mt-1 blur-[3px]">{item.meu_olhar}</p>
    </div>
  </button>
);
